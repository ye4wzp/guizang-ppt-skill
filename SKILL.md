...
description: 生成横向翻页网页 PPT（单 HTML 文件），提供四种风格：① "电子杂志 × 电子墨水"（衬线 + 流体背景 + 暖色） ② "瑞士国际主义"（无衬线 + 网格点阵 + 高亮色） ③ "日式侘寂"（极细宋体 + 和纸纹理 + 墨渍装饰） ④ "新丑主义"（超粗无衬线 + 粗描边 + 色块反叛）。当用户需要制作分享 / 演讲 / 发布会风格的网页 PPT 时使用。
---
...
生成一份**单文件 HTML**的横向翻页 PPT，提供四种可选的视觉基调：
...
- **和纸纹理背景 + 墨渍装饰点 + 竖排侧边文字**
- **极细宋体（Noto Serif SC 200 weight）+ EB Garamond 意大利体 + 等宽元数据**
- **非对称布局**：内容偏左 58%，右侧留白或放图，大量呼吸空间
- **慢速水墨淡入动效**（1.2s），像墨迹晕开
- 适合：茶道、花道、冥想、文学、传统文化、日式美学、极简主义分享
- 模板：`assets/template-wabi.html` · 主题色：`references/themes-wabi.md`
- 美学锚点：像原研哉 / MUJI / *Cereal* 杂志

### 风格 D · 新丑主义（Brutalism）

- **超粗无衬线（Inter 900）+ Space Mono 等宽，全大写英文标题**
- **3px 粗描边卡片 + 贴纸标签 + 故意溢出的装饰色块**
- **打破网格**：重叠、错位、旋转，视觉冲击优先
- **快速弹性动效**（0.4s），干脆利落
- 适合：音乐节、创意发布会、设计分享、独立杂志、反传统场景
- 模板：`assets/template-brutal.html` · 主题色：`references/themes-brutal.md`
- 美学锚点：像 David Carson / Saul Bass / 音乐节海报

**四种风格共享**：横向翻页（键盘 ← →、滚轮、触屏、点击、ESC 索引）、Lucide 图标、Motion One 入场动效（本地 + CDN 双保险）。
...
| # | 问题 | 为什么要问 |
|---|------|-----------|
| 1 | **风格选哪个?**(A 电子杂志风 / B 瑞士国际主义风 / C 日式侘寂风 / D 新丑主义风) | **必须先问**,决定用哪个 template + themes 文件 |
...
| "侘寂" / "日式" / "wabi-sabi" / "极简留白" / "茶道" / "MUJI" / "原研哉" | **C · 日式侘寂风** |
| "新丑" / "brutalism" / "反叛" / "音乐节" / "大胆" / "anti-design" / "David Carson" | **D · 新丑主义风** |
| 内容是 AI 产品 / 技术 / 工程 / 数据汇报 | B 更合适 |
| 内容是行业观察 / 人文 / 故事 / 文化 | A 更合适 |
| 内容是茶道 / 花道 / 传统文化 / 文学 / 日式美学 | C 更合适 |
| 内容是创意 / 设计 / 音乐 / 独立杂志 / 反传统 | D 更合适 |
...
# 或 风格 C · 日式侘寂风
cp "<SKILL_ROOT>/assets/template-wabi.html" "项目/XXX/ppt/index.html"

# 或 风格 D · 新丑主义风
cp "<SKILL_ROOT>/assets/template-brutal.html" "项目/XXX/ppt/index.html"
```

四个 `template*.html` 都是**完整可运行**的文件——CSS、翻页 JS、字体/图标 CDN 全已预设好,只有 `<!-- SLIDES_HERE -->` 占位符等待你填充 slide 内容。

**注意**:四种风格**不能混用**。每套模板有自己的 CSS 类名体系,一份 deck 只能选一套。
...
│   ├── template-wabi.html    ← 风格 C · 日式侘寂风模板（种子文件）
│   ├── template-brutal.html  ← 风格 D · 新丑主义风模板（种子文件）
...
└── references/
    ...
    ├── themes-wabi.md        ← 风格 C · 5 套侘寂风主题色预设（暖墨 / 枯山水 / 朱砂 / 青竹 / 月白）
    ├── themes-brutal.md      ← 风格 D · 4 套新丑风主题色预设（经典 / 单色 / 莫兰迪 / 热力）
...
> 侘寂风的灵魂是"不完美的美"——克制、留白、有呼吸感。

1. **极细宋体为王** — Noto Serif SC 200/300 weight，大标题必须细到有空气感
2. **非对称布局** — 内容偏左 58%，右侧留白或放图，不要居中对称
3. **和纸纹理** — 背景必须有和纸纹理叠加，纯白太刺眼
4. **墨渍装饰** — 小圆点、极细横线、竖线，位置随意但数量克制（每页 ≤ 3 个）
5. **慢速动效** — 1.2s 淡入，像水墨晕开，不要快速弹跳
6. **图片去饱和** — `contrast(.92) saturate(.85)`，不要太鲜艳
7. **呼吸感 > 信息量** — 宁可少放内容，不要塞满页面

### 风格 D · 新丑主义风

> 新丑风的灵魂是"故意打破规则"——粗粝、大胆、反精致。

1. **超粗无衬线** — Inter 900 weight，全大写英文标题，字号越大越好
2. **3px 粗描边** — 卡片边框 3px 是底线，不要削弱为 1px
3. **三原色同时出现** — 红/黄/蓝可以同时用，不要只留一个
4. **贴纸标签** — `.sticker` 组件带旋转角度（-2° ~ -1°），是新丑标志
5. **装饰块故意溢出** — 色块可以超出页面边界，不要完美对齐
6. **快速弹性动效** — 0.4s，干脆利落，不要慢速优雅
7. **直角到底** — 不允许圆角（`border-radius:0`），不允许阴影
8. **打破网格** — 允许重叠、错位、旋转，视觉冲击优先于秩序
